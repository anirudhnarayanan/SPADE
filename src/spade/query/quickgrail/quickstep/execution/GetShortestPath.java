...
public class GetShortestPath
	extends AbstractGetShortestPath<QuickstepGraph, QuickstepGraphMetadata, QuickstepEnvironment, Quickstep>{
	
	public GetShortestPath(QuickstepGraph targetGraph, QuickstepGraph subjectGraph,
	...
	public void execute(QuickstepEnvironment env, ExecutionContext ctx, Quickstep storage){
		String filter;
		storage.executeQuery("DROP TABLE m_conn;\n" +
				"CREATE TABLE m_conn (src INT, dst INT);");
		if(env.IsBaseGraph(subjectGraph))
		{
			filter = "";
			storage.executeQuery("\\analyzecount edge\n" +
		...
		else
		{
			...
			storage.executeQuery("DROP TABLE m_sgedge;\n" +
		...
		storage.executeQuery("\\analyze m_conn\n");
		...
		storage.executeQuery("INSERT INTO m_cur SELECT id, id FROM " + destinationGraph.getVertexTableName() + ";\n" +
				"\\analyzerange m_cur\n" +
